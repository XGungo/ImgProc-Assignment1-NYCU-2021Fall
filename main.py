...
class PixelOp:
    ...
    class ContrastStretching:
        ...
        def percentile(image: np.array, r1: float, r2: float, s1: float = 0, s2=255) -> np.array:
            ...
            result[np.multiply(image > r1, image < r2)] *= (s2 - s1) / (r2 - r1)
            result[np.multiply(image > r1, image < r2)] += (r2 * s1 - r1 * s2) / (r2 - r1)
...
class NeighborhoodOp:
    ...
    """
    @staticmethod
    def padding(image, k_size, padding_mode='replicate'):
        """
        Remaking the outer-frame of image makes the convolution easier.
        :param image: np.array
            Input image.
        ...
        assert (k_size % 2) == 1, 'k_size(Kernel Size) must be odd.'
        m, n = k_size, k_size
    ...
    def filtering(image, k_size=3, mode='median', **kwargs):
        ...
        if mode == 'median':
            ...
            if mode == 'gaussian':
                x, y = np.mgrid[-(k_size - 1) // 2:(k_size - 1) // 2 + 1, -(k_size - 1) // 2:(k_size - 1) // 2 + 1]
            ...
            if len(image.shape) == 3:
                ...
                for channel in range(image.shape[-1]):
                    padding_image = NeighborhoodOp.padding(image[:, :, channel], k_size, **kwargs)
                    result[:, :, channel] = NeighborhoodOp.convolution(image[:, :, channel], kernel, padding_image)
            else:
                padding_image = NeighborhoodOp.padding(image, k_size, **kwargs)
                result = NeighborhoodOp.convolution(image, kernel, padding_image)
            return result
...
if __name__ == '__main__':
    ...
    # min-Max
    minmax = PixelOp.ContrastStretching.percentile(img, r1=np.min(img), s1=0, r2=np.max(img), s2=255)
    Image.fromarray(minmax).convert('L').save('output/minmax.jpg')

    # min-Max with c
    c = 15
    minmax_c = PixelOp.ContrastStretching.percentile(img, r1=np.min(img) + c, s1=0, r2=np.max(img) - c, s2=255)
    ...
    cum = 1
    plt.hist(img.ravel(), bins=256, density=1, cumulative=cum, label='origin', histtype='step')
    plt.hist(minmax.ravel(), bins=256, density=1, cumulative=cum, label='min-Max', histtype='step')
    plt.hist(minmax_c.ravel(), bins=256, density=1, cumulative=cum, label=f'min-Max with c={c}', histtype='step')
    plt.hist(he.ravel(), bins=256, density=1, cumulative=cum, label='HE', histtype='step')
    ...
    Image.fromarray(sharpen_corner).convert('L').save('output/sharpening_corner.jpg')
    ...
    Image.fromarray(edge_night_corner).convert('L').save('output/edge_night_corner.jpg')

    edge_gaussian = img - NeighborhoodOp.filtering(img, mode='gaussian')
    Image.fromarray(edge_gaussian).convert('L').save('output/edge_gaussian.jpg')
